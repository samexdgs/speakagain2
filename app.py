    ...
    with tab_account:
        ...
        with col2:
            ...
            if google_oauth_configured():
                # Streamlit's st.login() redirects; on return, st.user.is_logged_in
                if st.button(
                    f"🔑 {t('auth.google', lang)}",
                    use_container_width=True, key="google_login_btn",
                    type="secondary",
                ):
                    # Streamlit-native OIDC call
                    try:
                        st.login()  # no arg = default provider from [auth]
                    except TypeError:
                        # Older Streamlit may need provider name
                        st.login("google")
            else:
                st.info(
