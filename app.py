...
# YarnGPT covers Yoruba, Igbo, Hausa, Pidgin natively.
# gTTS handles English, Spanish, French, Portuguese, Arabic natively.
YARNGPT_LANGUAGES = {"Yoruba", "Igbo", "Hausa", "Pidgin"}

# Voice mapping per language. YarnGPT voices are language-aware via the engine,
# but we pick a voice character that suits each language's typical cadence.
YARNGPT_VOICE_BY_LANG = {
    "Yoruba": "Idera",     # melodic, gentle
    "Igbo":   "Chinenye",  # engaging, warm
    "Hausa":  "Umar",      # calm, smooth
    "Pidgin": "Tayo",      # upbeat, energetic
}


def _get_yarngpt_api_key():
    """Read YarnGPT API key from secrets or env. None if not configured."""
    try:
        if "YARNGPT_API_KEY" in st.secrets:
            return st.secrets["YARNGPT_API_KEY"]
    except Exception:
        pass
    import os
    return os.environ.get("YARNGPT_API_KEY")


def _yarngpt_synthesize(text: str, voice: str) -> bytes | None:
    """Call YarnGPT TTS endpoint. Returns mp3 bytes or None on failure.

    Circuit breaker: after 2 consecutive failures in the same session we stop
    trying YarnGPT for the rest of the session and go straight to gTTS. This
    prevents every speak action from waiting on a failing API. The breaker
    resets when the user reloads the page (next session)."""
    # Circuit breaker check
    failures = st.session_state.get("_yarngpt_failures", 0)
    if failures >= 2:
        return None

    api_key = _get_yarngpt_api_key()
    if not api_key:
        return None
    try:
        import requests
        r = requests.post(
            ...
            json={
                "text": text[:2000],  # YarnGPT limit
                "voice": voice,
                "response_format": "mp3",
            },
            timeout=5,  # short timeout — gTTS fallback is fast enough
        )
        if r.status_code == 200 and r.content:
            # Success: reset the failure counter
            st.session_state["_yarngpt_failures"] = 0
            return r.content
        # Non-200: record a failure so we trip the breaker quickly
        st.session_state["_yarngpt_failures"] = failures + 1
    except Exception:
        st.session_state["_yarngpt_failures"] = failures + 1
    return None


def _gtts_synthesize(text: str, lang_code: str, tld: str) -> bytes | None:
    """gTTS fallback. Returns mp3 bytes or None on failure."""
...
def text_to_speech_html(text: str, lang: str = "English") -> str:
    """
    Synthesize speech for `text` in the given app language.
    Routes Nigerian languages to YarnGPT (native voices) and everything else
    to gTTS (which has native voices for EN/ES/FR/PT/AR). Falls back to gTTS
    if YarnGPT is unavailable, so the app never goes mute.
    """
    clean = (text or "").strip()
    if not clean:
        return ""
    ...
    # Try YarnGPT first for the 4 Nigerian languages
    if lang in YARNGPT_LANGUAGES:
        voice = YARNGPT_VOICE_BY_LANG.get(lang, "Idera")
        audio_bytes = _yarngpt_synthesize(clean, voice)

    # Fallback: gTTS for non-Nigerian languages, OR if YarnGPT failed
    if audio_bytes is None:
        audio_bytes = _gtts_synthesize(
            clean,
            lang_code=get_tts_lang_code(lang),
            tld=get_tts_tld(lang),
        )

    if audio_bytes is None:
        return "<div style='font-size:12px;color:#999'>Voice unavailable.</div>"
...
def speak(text: str):
    lang = get_lang()
    html = text_to_speech_html(text, lang=lang)
    if html:
...
def _handle_google_return():
    ...
    Account linking: register_or_fetch_google_user returns the canonical
    username for this email. If the user previously signed up with a
    password using the same email, that password username is returned and
    we land in the existing account (same profile, same assessment, same
    history). No duplicate accounts are created.
    """
    claims = google_current_user()
    if not claims:
        return False
    try:
        username, user_record = register_or_fetch_google_user(claims)
